# -*- coding: utf-8 -*-
# ╔══════════════════════════════════════════════════════════╗
# ║         CYBER SEARCHER v4.6 — FULL PRODUCTION           ║
# ║              Developer: @hackledin                       ║
# ║  🎵 Müzik + 🎥 Video (POT ile Bot Koruması Aşıldı)      ║
# ║  🆔 Telegram ID Sorgu (gettg.id API)                    ║
# ║  🎨 AI Image Generator (minifreeai.com) — YENİ!         ║
# ╚══════════════════════════════════════════════════════════╝
import telebot
import requests
import os
...
# ══════════════════════════════════════════════════════════════
#  🎨 AI IMAGE GENERATOR (minifreeai.com)
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
#  🎨 AI IMAGE GENERATOR - DB FONKSİYONLARI
# ══════════════════════════════════════════════════════════════
...
def aiimg_get_credits(user_id):
    return aiimg_get(user_id, "credit_balance") or 0

def aiimg_get_total(user_id):
    return aiimg_get(user_id, "total_generated") or 0
...
def aiimg_can_use(user_id):
    ...
    if free_used < AI_IMG_FREE_LIMIT:
        return True, "free"
...
def aiimg_use(user_id):
    ...
    if credits > 0:
        aiimg_set(user_id, "credit_balance", credits - 1)
        aiimg_set(user_id, "total_generated", aiimg_get_total(user_id) + 1)
        return True, "credits"
    return False, None

def aiimg_add_credits(user_id, amount):
    current = aiimg_get_credits(user_id)
    aiimg_set(user_id, "credit_balance", current + amount)
...
def aiimg_generate(image_path, prompt, max_tries=40, delay=2.5):
    try:
        try:
            ...
            return False, f"❌ Resim okunamadı: {e}"
        ...
        try:
            ...
            return False, f"❌ API geçersiz cevap: {response.text[:200]}"

        task_id = data.get("task_id")
        if not task_id:
            return False, f"❌ Task ID alınamadı.\nAPI cevabı: {str(data)[:200]}"
...
def _exif_mesaj_olustur(d: dict) -> str:
    ...
    msg += f"{'━' * 28}\n🤖 <i>Cyber Searcher v4.6 | @hackledin</i>"
    return msg
...
S = {
    "tr": {
        "welcome": "🌟 <b>Cyber Searcher v4.6</b>\nHoşgeldin, <b>{name}</b>!\n📌 Durum: {status}\n🔻 Aşağıdan işlem seç:",
        "free": "🆓 Ücretsiz",
        "premium": "⭐ PREMIUM",
        ...
        "help_content": (
            ...
            "   • ⭐ Premium: Sınırsız\n"
            "🔹 **🎨 AI IMAGE GENERATOR (YENİ!)**:\n"
            "   • 🆓 Free: 2 hak\n"
            "   • 💎 Paketler: 25→89⭐ / 50→200⭐ / 250→600⭐\n"
            "   • ⭐ Premium: Sınırsız\n"
            "   • 📸 Fotoğraf gönder → AI dönüştürsün!\n"
            "🔹 **⭐ PREMIUM PAKETLER:**\n"
            "   • 🌟 Premium (400 Yıldız) → Sınırsız Hotmail + Capture + Keyword + TG-ID + AI\n"
    ...
    "en": {
        ...
        "help_content": (
            ...
            "══════════════════════\n"
            "🔹 **⭐ PREMIUM PACKAGES:**\n"
            "   • 🌟 Premium (400 Stars) → Unlimited Hotmail + Capture + Keyword + TG-ID + AI\n"
...
def aiimg_kb(user_id):
    ...
    if user_id == ADMIN_ID:
        ...
        durum = f"🆓 Free: {free_left}/{AI_IMG_FREE_LIMIT}  |  💎 Hak: {credits}"
...
# ══════════════════════════════════════════════════════════════
#  🆔 TELEGRAM ID SORGU — gettg.id API + Normalize
# ══════════════════════════════════════════════════════════════
...
def tgid_api_search(username):
    try:
        ...
        # YENİ FORMAT: {"status": "success", "data": "{...}"}
...
def tgid_build_txt_report(username, data, queried_by=""):
    ...
    lines.append(" 🛡️ Cyber Searcher v4.6")
    lines.append(sep)
    return "\n".join(lines)
...
def aiimg_process(msg, bot_instance):
    ...
    if not allowed:
        ...
        bot_instance.reply_to(
            ...
            reply_markup=aiimg_packages_kb()
        )
        return

    if not msg.photo and not msg.document:
        bot_instance.reply_to(msg, "❌ Lütfen bir fotoğraf gönder!")
        return
    ...
    try:
        ...
        bot_instance.reply_to(msg, f"❌ Fotoğraf indirilemedi: {e}")
        return
    ...
    try:
        ...
        bot_instance.reply_to(msg, f"❌ Dosya kaydedilemedi: {e}")
        return
...
def aiimg_run(msg, temp_path, bot_instance):
    ...
    if not prompt:
        bot_instance.reply_to(msg, "❌ Prompt boş olamaz!")
        ...
        try:
            ...
            if not success:
                ...
                try:
                    ...
                    bot_instance.send_message(msg.chat.id, f"❌ {result}")
                return
            ...
            if uid == ADMIN_ID:
                ...
                hak = f"🆓 Free: {free_left}/{AI_IMG_FREE_LIMIT} | 💎 Hak: {credits}"
            ...
            try:
                ...
                if img_resp.status_code == 200:
                    bot_instance.send_photo(msg.chat.id, img_resp.content,
            ...
            try:
                bot_instance.edit_message_text(f"❌ Beklenmeyen hata: {e}", msg.chat.id, wait.message_id)
            except: pass
        finally:
            try: os.remove(temp_path)
            except: pass
...
def aiimg_show_stats(chat_id, uid, bot_instance):
    ...
    txt = (
        f"📊 <b>AI RESİM İSTATİSTİKLERİN</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    while HOTMAIL_QUEUE_RUNNING:
        try:
            ...
            result_lines = [
                "✅ **Tarama Tamamlandı!**",
                "━━━━━━━━━━━━━━━━━━━━━",
                f"📁 Dosya: hits_{task.user_id}.txt",
                f"📊 Toplam: {total}",
                "",
                f"✅ HIT: {HOTMAIL_HIT}",
                f"🎁 Rewards Hits: {HOTMAIL_REWARDS}",
                f"🔐 2FA: {HOTMAIL_2FA}",
                f"❌ BAD: {HOTMAIL_BAD}",
...
def get_queue_status_text(user_id: int = None) -> str:
    with HOTMAIL_QUEUE_LOCK:
        ...
        if queue_list:
            lines.append("")
            lines.append(f"  📊 **Sırada Bekleyenler ({len(queue_list)})**")
            lines.append("")
...
def register_handlers(bot_instance):
    ...
    def cmd_aiimg(msg):
        ...
        txt = (
            ...
            f"<b>Desteklenen:</b> JPG, PNG, WEBP, BMP"
        )
    ...
    def handle_photo_exif(msg):
        ...
        aiimg_trigger = any(
            ...
            for t in ("/aiimg", "aiimg", "/ai", "/generate", "/uret")
        )
        if aiimg_trigger:
    ...
    def handle_cb(call):
        try:
            ...
            if data.startswith("aiimg_buy_"):
                ...
                pkg_map = {
                    "25":  (AI_IMG_PACK_25,  AI_IMG_PRICE_25,  "25 Hak"),
    ...
    def payment_ok(msg):
        ...
        if payload.startswith("aiimg_"):
            ...
            if pkg_num in pkg_map:
                ...
                bot_instance.reply_to(
                    ...
                    f"Hemen resim üretmeye başlayabilirsin! 🎨",
                    parse_mode="HTML"
                )
                try:
                    bot_instance.send_message(
        ...
        if payload == "premium":
            ...
            bot_instance.reply_to(msg, "🎉 **Hotmail Premium aktif!**\n📧 Sınırsız Hotmail + 📸 Sınırsız Capture + 🔖 Sınırsız Keyword + 🆔 Sınırsız TG-ID + 🎨 Sınırsız AI erişimi kazandın.")
            bot_instance.send_message(
                ADMIN_ID,
...
def _handle_admin_cb(call, action, bot_instance):
    ...
    try:
        ...
        # 🎨 AI Image Admin
            ...
            conn.close()
            if not logs:
                bot_instance.send_message(cid, "📭 AI log kaydı yok.")
            else:
                txt = "📋 <b>SON 20 AI LOG</b>\n━━━━━━━━━━━━━━━━━━━━━\n"
                for u_id, uname, prompt, status, date in logs:
                    ico = "✅" if status == "OK" else "❌"
                    p = (prompt or "")[:40]
                    txt += f"{ico} @{uname or u_id} | {p} | {date[5:16]}\n"
                bot_instance.send_message(cid, txt[:4096])
            try: bot_instance.answer_callback_query(call.id)
            except: pass
            return
            ...
            conn.close()
            if not logs:
                bot_instance.send_message(cid, "📭 AI satın alma yok.")
            else:
                txt = "💰 <b>SON 20 AI SATIN ALMA</b>\n━━━━━━━━━━━━━━━━━━━━━\n"
                for u_id, uname, package, qty, stars, date in logs:
                    txt += f"👤 @{uname or u_id} | {package} | +{qty} | {stars}⭐ | {date[5:16]}\n"
                bot_instance.send_message(cid, txt[:4096])
            try: bot_instance.answer_callback_query(call.id)
            except: pass
            return
...
def _admin_aiimg_give(msg, bot_instance):
    ...
    aiimg_add_credits(target, amount)
    bot_instance.reply_to(
        msg,
        f"✅ <b>AI Resim Hakkı Verildi!</b>\n"
        f"🆔 Kullanıcı: <code>{target}</code>\n"
        f"➕ Miktar: <b>+{amount}</b>\n"
        f"💎 Yeni hak: <b>{aiimg_get_credits(target)}</b>",
        parse_mode="HTML"
    )
    try:
        bot_instance.send_message(
            target,
            f"🎁 <b>Admin sana AI Resim hakkı verdi!</b>\n"
            f"➕ Eklenen: <b>+{amount}</b> hak\n"
            f"💎 Yeni hakkın: <b>{aiimg_get_credits(target)}</b>",
            parse_mode="HTML"
        )
    except:
        pass
...
def _admin_aiimg_take(msg, bot_instance):
    ...
    try:
        ...
        bot_instance.reply_to(msg, "❌ Geçersiz format! Örnek: <code>123456789 10</code>")
        return
    current = aiimg_get_credits(target)
    new_val = max(0, current - amount)
    aiimg_set(target, "credit_balance", new_val)
    bot_instance.reply_to(
        msg,
        f"✅ <b>AI Resim Hakkı Alındı!</b>\n"
        f"🆔 Kullanıcı: <code>{target}</code>\n"
        f"➖ Miktar: <b>-{amount}</b>\n"
        f"💎 Yeni hak: <b>{new_val}</b>",
        parse_mode="HTML"
    )
...
if __name__ == "__main__":
    ...
    print("""
╔══════════════════════════════════════════════════════╗
║       CYBER SEARCHER v4.6 — PRODUCTION               ║
║         Developer: @hackledin                        ║
╠══════════════════════════════════════════════════════╣
